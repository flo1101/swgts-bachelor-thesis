...
const RESULTS_CSV = path.join(
  ...
  "upload-test-results",
  "socket",
  `socket_upload_performance_parallel.csv`,
);
...
for (let clientId = 1; clientId <= CLIENT_COUNT; clientId++) {
  test(`(Client ${clientId}): Socket upload test`, async ({ page }) => {
    ...
    const uploadTimes = await uploadTest(
      page,
      file,
    ...
    console.debug(
      `(Client ${clientId}): Average socket-upload performance for ${file.fileSizeMB} MB file: ${uploadTimes} seconds.`,
    );

    // Since state can't be shared between parallel running tests, each worker/test writes its result in the file directly
    console.debug(`(Client ${clientId}): Writing result to CSV.`);
    uploadTimes.forEach((uploadTime, i) => {
